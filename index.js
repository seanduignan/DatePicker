...
router.get('/get-profitdata', function(req, res, next) {
  UserProfitData.find()
      .then(function(doc) {
        res.render('index', {profits: doc});
      });
});


UserVetData.find( { date: { $gt: new Date('2018-01-01'), $lt: new Date('2018-12-31') } } )
.then(function(doc) {
var totalCost = 0;
varDocArray = doc.toArray();
for(var ii = 0; ii < varDocArray.length; ii++) {
totalCost += parseFloat(varDocArray[ii].cost);
}
